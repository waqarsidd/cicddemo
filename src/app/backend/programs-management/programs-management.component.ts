...
import { PROGRAM_FIELDS } from '@app/CONSTANT/program-fields';
...
@Component({
  ...
  styleUrls: ['./programs-management.component.css'],
})
export class ProgramsManagementComponent {
  public totalRecords: number;
  public isReqPending: boolean;
  public globalFilterFields = PROGRAM_FIELDS.filter(({ filter }) => filter);
  public searchControl: FormControl = new FormControl('');
  public searchedOptions: Array<any> = [];
  public columns: Array<IFormField> = PROGRAM_FIELDS.filter(({ read }) => read);
